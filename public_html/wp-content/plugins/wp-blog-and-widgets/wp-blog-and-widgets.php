               ...
               $terms = get_the_terms( $post->ID, 'blog-category' );
            	...
            	<div id="post-<?php the_ID(); ?>" class="blog type-blog <?php echo (has_post_thumbnail()) ? "has-thumb" : "no-thumb";?> blog-col-<?php echo $gridcol.' '.$css_class.' '.$date_class; ?>">
					...
					<?php if($showAuthor == 'true' || $showDate == 'true'){ ?>
					 ...
					 <span><?php esc_html_e( 'No', 'wp-blog-and-widgets' ); ?> <?php the_author(); ?>
					 </span>
					 <?php }?>
					...
					<?php } ?>
						...
						<?php if($gridcol == '1'){ ?>
						<div class="blog-cat">
                          <?php if($showAuthor == 'true') { ?> <span class="grid-1-author"><?php _e( 'No', 'wp-blog-and-widgets' ); ?> <?php the_author(); ?></span>  <?php } echo ($showAuthor == 'true' && $showCategory == 'true') ? '/' : '' ?> <?php if($showCategory == 'true') { echo $cate_name; } ?>
							</div>
                       <?php }?>
                    ...
                    <p class="blog-short-content"><?php echo blog_limit_words( $post->ID, $excerpt, $words_limit, '...'); ?></p>
                        ...
                        <a href="<?php the_permalink(); ?>" class="more-link"> <?php _e( 'Lasīt vairāk', 'wp-blog-and-widgets' ); ?></a>	
						<?php } else { 
							the_content();
						 } ?>
					</div><!-- .entry-content -->
