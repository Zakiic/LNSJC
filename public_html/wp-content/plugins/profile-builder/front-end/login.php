		...
		if( is_wp_error( $user ) ) {
            ...
            if (isset($wppb_generalSettings['loginWith'])) {
                ...
                if ($user->get_error_code() == 'incorrect_password') {
                    ...
                    $error_string .= '<a href="' . $LostPassURL . '" title="' . __('Parole Lost and Found.', 'profile-builder') . '">' . __('Lost your password', 'profile-builder') . '</a>?';
...
function wppb_front_end_login( $atts ){
	...
	if( !is_user_logged_in() ){
		...
		if ( isset( $wppb_generalSettings['loginWith'] ) && ( $wppb_generalSettings['loginWith'] == 'usernameemail' ) )
			$form_args['label_username'] = __( 'E-pasts', 'profile-builder' );
