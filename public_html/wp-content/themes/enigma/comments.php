...
<?php if ( comments_open() ) : ?>
	...
	add_filter('wl_comment_form_default_fields','my_fields');
		...
		'comment_field'=> '<div class="enigma_form_group"><label for="message"> Komentāri</label>
		<textarea id="comment" name="comment" class="enigma_con_textarea_control" rows="5"></textarea></div>',		
		'logged_in_as' => '<p class="logged-in-as">' . __( " ",'weblizar' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="Log out of this account">'.__(" ",'weblizar').'</a>' . '</p>',
		'title_reply_to' => __( 'Komentāri %s','weblizar'),
		'id_submit' => 'enigma_send_button',
		'label_submit'=>__( 'Pievieno komentāru','weblizar'),
		'comment_notes_before'=> '',
		'comment_notes_after'=>'',
		'title_reply'=> '<h2>'.__('Komentāri','weblizar').'</h2>',		
		'role_form'=> 'form',		
		);
